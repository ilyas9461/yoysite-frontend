...
export default class LoginSevice {
      ...
      try {
         ...
         user = await axios
            .get(URL + "/user/userdata", {
               //tokendeki kullanıcı verileri alınıyor.
         ...
         localStorage.setItem("user", JSON.stringify(user.data)); //user yerel depoya kaydediliyor
         //console.log('Local storage getLoginData(): ', localStorage.getItem('user'));
   ...
   async getFirmaData(firmaWebId) {
      ...
      const res = await apiClient
         .post("/user/firma", JSON.stringify(jsonData), {
